        ...
        success: function(cinemas){
            ...
            if(cs.length > 0){
                ...
                for(i = 0; i<cs.length; i++){
                    ...
                    $('#cinemasList').append("<tr><th scope='row'>"+cs[i].id+"</th><td>"+cs[i].owner+"</td><td>"+cs[i].capacity+"</td><td>"+cs[i].category_id+"</td><td>"+cs[i].name+"</td><td>"+"<button class='btn btn-warning' onclick='deleteCinema("+cs[i].id+")' data-toggle='modal' data-target='#messageModal'>EDIT</button> <button class='btn btn-danger' onclick='deleteCinema("+cs[i].id+")' data-toggle='modal' data-target='#messageModal'>DELETE</button></td></tr>");
